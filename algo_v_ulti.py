...
def preempt_traffic_signals(route, preempted_signals):
    for tls in traci.trafficlight.getIDList():
        try:
            ...
            print(f"[WARN] Error updating signal {tls}: {e}")
...
def main():
    ...
    print(f"Source exists in graph: {source in graph}")
    print(f"Destination exists in graph: {destination in graph}")
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        if current_edge == destination:
            reaching_time = traci.simulation.getTime()
            print(f"🎯 Destination edge {destination} reached at time {reaching_time}")
            break
        time.sleep(0.1)

    if reaching_time:
        total_time_taken = reaching_time - resume_time
        print(f"⏱️ Ambulance travel time to destination: {total_time_taken:.2f} seconds")
    else:
        print("⚠️ Ambulance did not reach destination.")
