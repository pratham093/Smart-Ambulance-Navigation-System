...
def heuristic_func(u, v):
    try:
        return abs(traci.edge.getLength(u) - traci.edge.getLength(v))
    except:
        return 1.0

def compute_route_astar(graph, source, destination):
    try:
        return nx.astar_path(graph, source, destination, heuristic=heuristic_func, weight='weight')
...
def main():
    ...
    route = compute_route_astar(graph, edge, DEST_EDGE)
    if route:
        traci.vehicle.setRoute(AMBULANCE_ID, route)
        print("📍 Initial A* route set.")
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        if now - last_reroute >= 10:
            ...
            new_route = compute_route_astar(graph, cur, DEST_EDGE)
            if new_route:
                ...
                print(f"⚠️ [Re‑routing @ {now:.1f}s] A* failed, skipping update")
            last_reroute = now
